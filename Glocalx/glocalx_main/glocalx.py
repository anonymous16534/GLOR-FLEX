...
from Glocalx.glocalx_main.callbacks import final_rule_dump_cb as final_rule_dump_callback
...
class GLocalX:
    ...
    def fit(self, rules, tr_set, batch_size=128, global_direction=False,
        ...
        if self.oracle is not None:
        	# y = self.oracle.predict(tr_set).round().astype(int).reshape(1, -1)
    ...
    def rules_1(self, alpha=0.5, data=None, evaluator=None, is_percentile=False, strategy='fidelity'):
        """
        Return the fine boundary of this instance, filtered by `alpha`.
        Args:
            alpha (Union(float | int)): Pruning factor, set to None for no pruning. Defaults to 0.5.
        ...
        if data is None:
            ...
            return fine_boundary_1

        return None



    def rules_0(self, alpha=0.5, data=None, evaluator=None, is_percentile=False, strategy='fidelity'):
        """
        Return the fine boundary of this instance, filtered by `alpha`.
        Args:
            alpha (Union(float | int)): Pruning factor, set to None for no pruning. Defaults to 0.5.
